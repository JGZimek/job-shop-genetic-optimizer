...
class SidebarFrame(ctk.CTkFrame):
    ...
    Fix: Adjusted wraplength to fit warning text within margins.
    """
    ...
    def _setup_algo_section(self):
        ...
        self.exact_warning = ctk.CTkLabel(
            self.scrollable_frame, 
            text="Warning: Not recommended for instances > 4x3. High complexity may cause freeze.", 
    ...
    def _create_param_field(self, param_key):
        ...
        ctk.CTkLabel(frame, text=f"{display_name}:", text_color="#b0b8c3", font=("Segoe UI", 11)).pack(anchor="w", pady=(0, 0))
    ...
    def _update_param_visibility(self):
        ...
        if self.selected_algorithm == "genetic":
            ...
            # padx=20 gives nice indentation, wraplength handles the rest
            self.exact_warning.pack(
                in_=self.scrollable_frame, 
                before=self.algo_separator, 
                fill="x", 
                pady=(5, 5), 
                padx=20 
            )
    ...
    def _validate_field_live(self, key):
        ...
        try:
            ...
            if soft_limit and val > soft_limit:
                ...
                entry.configure(border_color=COLOR_NORMAL)
        except ValueError:
            entry.configure(border_color=COLOR_ERROR)
    ...
    def get_parameters(self):
        ...
        for key, widget in self.params_widgets.items():
            ...
            try:
                ...
                if constraint["min"] is not None and val < constraint["min"]:
                    raise ValueError(f"Min: {constraint['min']}")
                if constraint["max"] is not None and val > constraint["max"]:
                    raise ValueError(f"Max: {constraint['max']}")
        ...
        if warnings_list:
            dialog = StatusDialog(self.winfo_toplevel(), "Performance Warning", "High complexity params detected:", "\n".join(warnings_list), type_="warning")
            if not dialog.result: return None 
